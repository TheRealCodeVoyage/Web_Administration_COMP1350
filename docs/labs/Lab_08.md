---
title: "Lab 8: Eliminating Single Points of Failure"
---

[&larr; Back to course index]({{ '/' | relative_url }})

{% raw %}
# Lab 8: Eliminating Single Points of Failure

*COMP 1350 — Web Administration, Week 10*

Last week you built a load-balanced setup — but look closely and it still has single points of failure. In this lab you'll map every SPOF in your Lab 7 architecture, then redesign and rebuild it so the load balancer itself is no longer the weak link.

> **Apple Silicon (M1/M2/M3/M4) Mac?** If you used the VMware Fusion / arm64 path for Lab 7, keep using it here — add `lb2` in Part 3 with the same arm64 box and `vmware_desktop` provider block as your other VMs, and use `vagrant up lb2 --provider=vmware_desktop`.

## Part 1: Audit Your Current Architecture

1. Reopen your Lab 7 project directory and bring the VMs back up if needed:
...
2. Draw (on paper, in a slide, or in any diagramming tool) your current architecture from Lab 7: client → `lb` (HAProxy) → `backend1` / `backend2`.

3. For each component, mark whether it is a SPOF (single point of failure) and why:

| Component | Is it a SPOF? | Why / Why not |
|---|---|---|
| `backend1` or `backend2` alone | No | Load balancer routes around a failed one (you proved this in Lab 7, Part 6) |
...
**Q2**: Both backends are up, yet the site is unreachable. In one sentence, explain why removing the app-server SPOF in Lab 7 didn't actually remove *all* single points of failure from the system.
...
```
   ...
   Use the same `frontend`/`backend` block from Lab 7, Part 3, pointing at the same two backends.
...
Compare this explicitly against your Lab 7 "before" diagram from Part 1.
...
- Your "before" (Lab 7) and "after" (this lab) architecture diagrams
